...
def init(project_id: str, topic: str, pipeline_topic: str = "tft-pipeline-events"):
    global _publisher, _project_id, _topic, _pipeline_topic
    _project_id     = project_id
    _topic          = topic
    _pipeline_topic = pipeline_topic
...
def publish_pipeline_event(payload: dict):
    """Publica evento em tft-pipeline-events para triggar o dbt Runner."""
    message = json.dumps(payload).encode("utf-8")
    future  = _publisher.publish(_topic_path(_pipeline_topic), message)
    future.result()
    logger.info(f"Evento publicado em {_pipeline_topic}: {payload.get('event')}")


def publish_batch(match_ids: list[str], batch_num: int):
    """Publica um batch de match IDs como uma única mensagem Pub/Sub."""
    payload = json.dumps({
        "match_ids": match_ids,
    ...
    future = _publisher.publish(_topic_path(_topic), payload)
    future.result()
